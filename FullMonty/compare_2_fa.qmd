...
my_plot_chart <- function(pv,
  ...
  p <- plot(pv) +  # Start building the plot here
    common_scale +
    ggtitle(title)
  ...
  if(!is.null(coastlines)) {
    ...
    p <- p + geom_path(aes(x, y), data=coastlines, colour = "black")
  }
...
file_key_1 <- paste(path_1, exp_1, "PGD_prel.fa",sep="/")
file_key_2 <- paste(path_1, exp_1, "PGD_prel.fa",sep="/")
#file_key_2 <- paste(path_1, exp_1, "archive/2025/01/15/09/mbr000/ICMSHHARM+0006.sfx",sep="/")
...
fa_name_1 <- str_pad(paste("SFX.ZS"), 16, 'right', pad=' ')
fa_name_2 <- str_pad(paste("SFX.MAX_ZS"), 16, 'right', pad=' ')
...
ZS <- Inventory_1[grep("ZS", Inventory_1)]
}
...
chart_breaks = c(0, 50, 100, 500,  1000, 1500, 2000, 2500)
```
